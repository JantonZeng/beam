...
class OtherPersonAgentSpec
  ...
  private lazy val parkingManager = system.actorOf(
    ZonalParkingManager.props(beamSvc, beamSvc.beamRouter, ParkingStockAttributes(100)),
    "ParkingManager")
  ...
  describe("A PersonAgent FSM") {
    it("should also work when the first bus is late") {
      ...
      val bus = new BeamVehicle(beamVehicleId, new Powertrain(0.0), BeamVehicleType.defaultCarBeamVehicleType)
      val tram =
        new BeamVehicle(Id.createVehicleId("my_tram"), new Powertrain(0.0), BeamVehicleType.defaultCarBeamVehicleType)
      ...
      bus.becomeDriver(
        Await.result(system.actorSelection("/user/router/TransitDriverAgent-my_bus").resolveOne(), timeout.duration))
      tram.becomeDriver(
        Await.result(system.actorSelection("/user/router/TransitDriverAgent-my_tram").resolveOne(), timeout.duration))
      ...
      lastSender ! ReservationResponse(
        ...
        Right(ReserveConfirmInfo(busLeg.beamLeg, busLeg2.beamLeg, reservationRequestBus.passengerVehiclePersonId)),
        TRANSIT)
      expectMsgType[PersonEntersVehicleEvent]
