    ...
    "contains valid mode chosen stats" in {
      val waitingStat =
        new ModeChosenAnalysis.ModeChosenComputation with EventAnalyzer {
          ...
          override def compute(stat: Tuple[java.util.Map[Integer, java.util.Map[String, Integer]], util.Set[String]])
              : Array[Array[Double]] = {
          ...
          override def eventFile(iteration: Int): Unit = {
            ...
            promise.future.foreach { a =>
              val modes = handler.counterValue.groupBy(identity).map { case (mode, ms) => mode -> ms.size }

              val all =
                a.asScala.values.flatMap(_.asScala).groupBy(_._1).map { case (s, is) => s -> is.map(_._2.toInt).sum }
      ...
      GraphRunHelper(new AbstractModule() {
        ...
        @Provides def provideGraph(eventsManager: EventsManager): ModeChosenStatsGraph = {
