  ...
  override def createNewScoringFunction(person: Person): ScoringFunction = {
    new ScoringFunction {
      ...
      override def handleEvent(event: Event): Unit = {
        event match {
          ...
          case e: PersonArrivalEvent =>
            ...
            trips.update(trips.size - 1, PersonAgent.correctTripEndTime(trips.last, e.getTime().toInt, bodyVehicleId))
          case _ =>
        }
      }
      ...
      private def writeTripScoresToCSV(): Unit = {
        ...
        val tripScoreData = trips.zipWithIndex
          .map { tripWithIndex =>
      ...
      private def registerLinkCosts(
          ...
          modeChoiceMultinomialLogit: ModeChoiceMultinomialLogit): Unit = {
            ...
            trip.legs.foreach { leg =>
              ...
              linksAndTravelTimes.foreach {
                case (linkId, linkTT) =>
                  ...
                  BeamScoringFunctionFactory.linkAverageCosts.put(linkId, newCostsAverage -> newCostsCount)
                  ...
                  BeamScoringFunctionFactory.linkAverageGeneralizedTimes.put(
                    linkId,
                    newGeneralizedTimesAverage -> newGeneralizedTimesCount) // NOTE: Store in seconds, not hours
              }
            }
        }
      }
...
object BeamScoringFunctionFactory extends OutputDataDescriptor {
  ...
  override def getOutputDataDescriptions: java.util.List[OutputDataDescription] = {
    val filePath =
      GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getIterationFilename(0, agentTripScoreFileBaseName + ".csv.gz")
    ...
    List(
      ...
      "score" -> "Trip score calculated based on the scoring function").map {
