...
class ModeChoiceLCCM(val beamServices: BeamServices, val lccm: LatentClassChoiceModel) extends ModeChoiceCalculator {
  ...
  private def choose(
      alternatives: IndexedSeq[EmbodiedBeamTrip],
      attributesOfIndividual: AttributesOfIndividual,
      tourType: TourType): Option[EmbodiedBeamTrip] = {
    if (alternatives.isEmpty) {
      ...
      val modeChoiceInputData = bestInGroup.map { alt =>
        val theParams =
          Map("cost" -> alt.cost, "time" -> (alt.walkTime + alt.bikeTime + alt.vehicleTime + alt.waitTime))
      ...
      val classMembershipInputData =
        lccm.classMembershipModels.head._2.alternativeParams.keySet.map { theClassName =>
          val modeChoiceExpectedMaxUtility =
            lccm.modeChoiceModels(tourType)(theClassName).getExpectedMaximumUtility(modeChoiceInputData)
       ...
       */
      val chosenClassOpt =
        lccm.classMembershipModels(tourType).sampleAlternative(classMembershipInputData, new Random())
      ...
      chosenClassOpt match {
        ...
        case Some(chosenClass) =>
          val chosenModeOpt =
            lccm.modeChoiceModels(tourType)(chosenClass).sampleAlternative(modeChoiceInputData, new Random())
          expectedMaximumUtility =
            lccm.modeChoiceModels(tourType)(chosenClass).getExpectedMaximumUtility(modeChoiceInputData)
  ...
  def sampleMode(
      ...
      tourType: TourType): Option[String] = {
    val bestInGroup = altsToBestInGroup(alternatives, tourType)
    val modeChoiceInputData = bestInGroup.map { alt =>
      val theParams = Map("cost" -> alt.cost, "time" -> (alt.walkTime + alt.bikeTime + alt.vehicleTime + alt.waitTime))
      AlternativeAttributes(alt.mode.value, theParams)
    }
  ...
  def utilityOf(embodiedBeamTrip: EmbodiedBeamTrip, conditionedOnModalityStyle: String, tourType: TourType): Double = {
    ...
    utilityOf(
      ...
      tourType,
      best.cost,
      getGeneralizedTime(best.walkTime + best.waitTime + best.vehicleTime + best.bikeTime, Some(best.mode)) * beamServices.beamConfig.beam.agentsim.agents.modalBehaviors.defaultValueOfTime)
  }

  def altsToBestInGroup(alternatives: IndexedSeq[EmbodiedBeamTrip], tourType: TourType): Vector[ModeChoiceData] = {
    ...
    val modeChoiceAlternatives: Seq[ModeChoiceData] =
      alternatives.zipWithIndex.map { altAndIdx =>
        ...
        val walkTime = altAndIdx._1.legs.view.filter(_.beamLeg.mode == WALK).map(_.beamLeg.duration).sum
        val bikeTime = altAndIdx._1.legs.view.filter(_.beamLeg.mode == BIKE).map(_.beamLeg.duration).sum
    ...
    val bestInGroup = groupedByMode.map {
      case (_, alts) =>
        ...
        alts
          .map { alt =>
            ((alt.vehicleTime + alt.walkTime + alt.waitTime + alt.bikeTime) / 3600 * 18 + alt.cost, alt)
          }
          .minBy(_._1)
          ._2
    }
  ...
  override def computeAllDayUtility(
    ...
    val vectorOfUtilities = List("class1", "class2", "class3", "class4", "class5", "class6")
      .map { style =>
        style -> beamServices.modeChoiceCalculatorFactory(attributesOfIndividual.copy(modalityStyle = Some(style)))
      }
      .toMap
    ...
    val logsum = Option(
      math.log(
        person.getPlans.asScala.view
          ...
          .sum)).filterNot(x => x < -100d).getOrElse(-100d)
