...
//          log.debug("Household {} has {} CAVs and will do some planning", household.getId, cavs.size)
          ...
          val cavScheduler = new FastHouseholdCAVScheduling(
            ...
            limitCavToXPersons = Int.MaxValue)(beamServices.matsimServices.getScenario.getPopulation)
                ...
                if (passengersToAdd.contains(person)) {
                  ...
                  if (pickDropsForGrouping.contains(person)) {
                    ...
                    personAndActivityToLegs = personAndActivityToLegs + ((
                        person.personId,
                        serviceRequest.pickupRequest.get.activity) -> legs)
          ...
          Future
            .sequence(
              cavPassengerSchedules
                ...
                .map { cavAndSchedule =>
                  akka.pattern
                    .ask(cavAndSchedule._1.driver.get, ModifyPassengerSchedule(cavAndSchedule._2, tick))
        ...
        personAndActivityToLegs.get((person.personId, originActivity)) match {
          case Some(legs) =>
            ...
            sender() ! CavTripLegsResponse(
              Some(cav),
              legs.map(bLeg =>
                EmbodiedBeamLeg(bLeg.copy(mode = CAV), cav.id, cav.beamVehicleType.id, false, 0.0, false, false)))
          case _ =>
