...
object PersonAgent {
  ...
  def props(
      ...
      routeHistory: RouteHistory): Props = {
    Props(
      new PersonAgent(
        scheduler,
        services,
  ...
  case class LiterallyDrivingData(delegate: DrivingData, legEndsAt: Double) extends DrivingData { // sorry
    ...
    def withCurrentLegPassengerScheduleIndex(currentLegPassengerScheduleIndex: Int) =
      LiterallyDrivingData(delegate.withCurrentLegPassengerScheduleIndex(currentLegPassengerScheduleIndex), legEndsAt)
  ...
  def correctTripEndTime(trip: EmbodiedBeamTrip, endTime: Int, bodyVehicleId: Id[BeamVehicle]) = {
    if (trip.tripClassifier != WALK && trip.tripClassifier != WALK_TRANSIT) {
      trip.copy(
        legs = trip.legs.dropRight(1) :+ EmbodiedBeamLeg
...
class PersonAgent(
  ...
  when(Uninitialized) {
    case Event(TriggerWithId(InitializeTrigger(_), triggerId), _) =>
      goto(Initialized).replying(CompletionNotice(triggerId, Vector(ScheduleTrigger(ActivityStartTrigger(0), self))))
  }
  ...
  when(Initialized) {
    case Event(TriggerWithId(ActivityStartTrigger(tick), triggerId), data: BasePersonData) =>
      ...
      goto(PerformingActivity).replying(
        CompletionNotice(
          triggerId,
  ...
  when(WaitingForDeparture) {
    ...
    case Event(
      ...
      eventsManager.processEvent(
        new PersonDepartureEvent(tick, id, currentActivity(data).getLinkId, currentTrip.tripClassifier.value))
      holdTickAndTriggerId(tick, triggerId)
  ...
  def handleFailedRideHailReservation(
    ...
    goto(ChoosingMode).using(
      ChoosesModeData(
        ...
        currentLocation =
          SpaceTime(beamServices.geo.wgs2Utm(data.restOfCurrentTrip.head.beamLeg.travelPath.startPoint).loc, tick),
  ...
  when(Waiting) {
    ...
    case ev @ Event(
      ...
      goto(Moving)
        .replying(CompletionNotice(triggerId))
        .using(data.copy(currentVehicle = vehicleToEnter +: currentVehicle))
  }
  ...
  when(Moving) {
    ...
    case Event(
      ...
      holdTickAndTriggerId(tick, triggerId)
      goto(ProcessingNextLegOrStartActivity).using(
        data.copy(
  ...
  when(ReadyToChooseParking, stateTimeout = Duration.Zero) {
    case Event(
      ...
      goto(ChoosingParkingSpot).using(
        data.copy(restOfCurrentTrip = theRestOfCurrentTrip, currentTripCosts = currentCost + completedLeg.cost))
  }
  ...
  when(ProcessingNextLegOrStartActivity, stateTimeout = Duration.Zero) {
      ...
      def nextState: FSM.State[BeamAgentState, PersonData] = {
        val currentVehicleForNextState =
          if (currentVehicle.isEmpty || currentVehicle.head != nextLeg.beamVehicleId) {
            ...
            eventsManager.processEvent(
              new PersonEntersVehicleEvent(_currentTick.get, Id.createPersonId(id), nextLeg.beamVehicleId))
    ...
    case Event(StateTimeout, data @ BasePersonData(_, _, nextLeg :: _, _, _, _, _, _, _, _, _))
      ...
      goto(ChoosingMode).using(
        ChoosesModeData(
          personData = data.copy(
            currentTourMode = Some(WALK_TRANSIT),
    ...
    case Event(
      ...
      nextActivity(data) match {
        case Some(activity) =>
          ...
          eventsManager.processEvent(
            new TeleportationArrivalEvent(tick, id, currentTrip.legs.map(l => l.beamLeg.travelPath.distanceInM).sum))
          ...
          eventsManager.processEvent(
            new ActivityStartEvent(tick, id, activity.getLinkId, activity.getFacilityId, activity.getType))
          scheduler ! CompletionNotice(triggerId, Vector(ScheduleTrigger(ActivityEndTrigger(endTime.toInt), self)))
          goto(PerformingActivity).using(
            data.copy(
  ...
  def handleSuccessfulReservation(
      ...
      data: BasePersonData,
      travelProposal: Option[TravelProposal] = None): FSM.State[BeamAgentState, PersonData] = {
