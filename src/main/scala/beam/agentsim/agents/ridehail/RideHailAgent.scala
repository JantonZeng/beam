...
object RideHailAgent {
  ...
  case class NotifyVehicleResourceIdleReply(triggerId: Option[Long], newTriggers: Seq[ScheduleTrigger])
...
class RideHailAgent(
  ...
  val myUnhandled: StateFunction = {
    ...
    case event @ Event(_, _) =>
      log.error("unhandled event: {} in state [ {} ] - vehicle( {} )", event.toString, stateName, vehicle.id.toString)
      stay()
  ...
  when(Uninitialized) {
    case Event(TriggerWithId(InitializeTrigger(tick), triggerId), data) =>
      ...
      if (isTimeForShift) {
        ...
        goto(Idle).using(data.copy(currentVehicle = Vector(vehicle.id), remainingShifts = shifts.getOrElse(List())))
      } else {
        val nextShiftStartTime = shifts.get.head.lowerBound
        goto(Offline)
          .replying(CompletionNotice(triggerId, Vector(ScheduleTrigger(StartShiftTrigger(nextShiftStartTime), self))))
          .using(data.copy(currentVehicle = Vector(vehicle.id), remainingShifts = shifts.get))
      }
  }
  when(Offline) {
    case Event(TriggerWithId(StartShiftTrigger(tick), triggerId), _) =>
    ...
    case ev @ Event(TriggerWithId(EndRefuelTrigger(tick, sessionStart, energyInJoules), triggerId), data) =>
  ...
  when(Idle) {
    case Event(TriggerWithId(EndShiftTrigger(tick), triggerId), data @ RideHailAgentData(_, _, _, _, _)) =>
    ...
    case ev @ Event(ModifyPassengerSchedule(updatedPassengerSchedule, tick, requestId), data) =>
      ...
      val triggerToSchedule = Vector(
        ScheduleTrigger(
          ...
          self))
      goto(WaitingToDriveInterrupted)
        .using(data.withPassengerSchedule(updatedPassengerSchedule).asInstanceOf[RideHailAgentData])
  ...
  def handleStartRefuel(tick: Int, triggerId: Long) = {
    ...
    stay().replying(
      CompletionNotice(
        triggerId,
        Vector(ScheduleTrigger(EndRefuelTrigger(tick + sessionDuration.toInt, tick, energyDelivered), self))))
  }
  ...
  def handleNotifyVehicleResourceIdleReply(ev: NotifyVehicleResourceIdleReply, data: RideHailAgentData) = {
      ...
      nextNotifyVehicleResourceIdle match {
        ...
        case Some(nextIdle) =>
          _currentTriggerId.foreach(
            log.debug("state(RideHailingAgent.awaiting NotifyVehicleResourceIdleReply) - triggerId: {}", _))
