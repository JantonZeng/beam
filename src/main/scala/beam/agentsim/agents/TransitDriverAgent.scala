      ...
      goto(WaitingToDrive)
        .using(
          data
            ...
            .asInstanceOf[TransitDriverData])
        .replying(CompletionNotice(
          triggerId,
          Vector(
            ScheduleTrigger(StartLegTrigger(schedule.schedule.firstKey.startTime, schedule.schedule.firstKey), self))))
  }
