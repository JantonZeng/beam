...
case class ExperimentRunner()(implicit experimentData: SigoptExperimentData) extends BeamHelper {
  ...
  def runExperiment(numberOfIterations: Int): Unit = {
    ...
    (0 to numberOfIterations).foreach { iter =>
      ...
      logger.info(
        logExpHelper(s"BEAM run completed for suggestion ${suggestion.getId} in ${execTimeInMillis / 1000} seconds"))

      val x = getRunValue(matsimConfig)

      val obs = new Observation.Builder().suggestion(suggestion.getId).value(x).build()
  ...
  def getRunValue(runConfig: MatsimConfig): Double = {
    ...
    if (objectiveFunctionClassName.equals("CountsObjectiveFunction")) {
      ...
      new ModeChoiceObjectiveFunction(benchmarkData.toAbsolutePath.toString)
        .evaluateFromRun(outpath.toAbsolutePath.toString, ErrorComparisonType.AbsoluteError)
                 ...
                 "ModeChoiceObjectiveFunction_AbsolutErrorWithPreferrenceForModeDiversity") && benchmarkFileExists) {
      val outpath = Paths.get(GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getOutputFilename("modeChoice.csv"))
      new ModeChoiceObjectiveFunction(benchmarkData.toAbsolutePath.toString).evaluateFromRun(
        outpath.toAbsolutePath.toString,
        ErrorComparisonType.AbsoluteErrorWithPreferenceForModeDiversity)
    } else if (objectiveFunctionClassName.equals(
                 "ModeChoiceObjectiveFunction_AbsoluteErrorWithMinLevelRepresentationOfMode") && benchmarkFileExists) {
      val outpath = Paths.get(GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getOutputFilename("modeChoice.csv"))
      new ModeChoiceObjectiveFunction(benchmarkData.toAbsolutePath.toString).evaluateFromRun(
        outpath.toAbsolutePath.toString,
    ...
    } else if (objectiveFunctionClassName.equals("RideHail_maximizeReservationCount")) {
  ...
  def createConfigBasedOnSuggestion(suggestion: Suggestion)(implicit experimentData: SigoptExperimentData): Config = {
    ...
    val runDirectory = experimentData.experimentDef.projectRoot
      .relativize(Paths.get(experimentBaseDir.toString, "experiments", experimentName, "suggestions"))

    val beamOutputDir: Path =
      experimentData.experimentDef.projectRoot.relativize(Paths.get(runDirectory.toString, suggestionId).toAbsolutePath)
    ...
    (Map(
      ...
      "beam.inputDirectory" -> experimentData.experimentDef.getTemplateConfigParentDirAsString) ++ configParams)
      .foldLeft(experimentData.baseConfig) {
