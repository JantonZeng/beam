...
private[vehiclesharing] class InexhaustibleReservingFleetManager(
  ...
  override def receive: Receive = {
    ...
    case MobilityStatusInquiry(_, whenWhere, _) =>
      ...
      val vehicle =
        new BeamVehicle(Id.createVehicleId(self.path.name + "-" + nextVehicleIndex), new Powertrain(0.0), vehicleType)
      nextVehicleIndex += 1
  ...
  def parkingInquiry(whenWhere: SpaceTime) =
    ParkingInquiry(whenWhere.loc, whenWhere.loc, "wherever", AttributesOfIndividual.EMPTY, NoNeed, 0, 0)
