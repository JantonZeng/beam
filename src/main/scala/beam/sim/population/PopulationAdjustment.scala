...
// check if excluded modes is defined for all individuals
    val allAgentsHaveAttributes = population.getPersons.asScala.forall { entry =>
      val personExcludedModes =
        Option(population.getPersonAttributes.getAttribute(entry._1.toString, PopulationAdjustment.EXCLUDED_MODES))
          .map(_.toString)
    ...
    allExcludedModes.groupBy(x => x).foreach(t => logger.info(s"${t._1} -> ${t._2.length}"))
  ...
  protected def existsMode(population: MPopulation, personId: String, modeToCheck: String): Boolean = {
    AvailableModeUtils
      .availableModesForPerson(population.getPersons.get(Id.createPersonId(personId)))
...
object PopulationAdjustment extends LazyLogging {
  ...
  def createAttributesOfIndividual(
      ...
      person: Person): AttributesOfIndividual = {
    ...
    val income =
      Option(personAttributes.getAttribute(person.getId.toString, "income")).map(_.asInstanceOf[Double]).getOrElse(0d)
    ...
    val valueOfTime: Double =
      Option(personAttributes.getAttribute(person.getId.toString, "valueOfTime"))
        ...
        .getOrElse(IncomeToValueOfTime(householdAttributes.householdIncome)
