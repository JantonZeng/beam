...
class RideHailFleetInitializer extends LazyLogging {
   ...
   */
  def init(beamServices: BeamServices): List[RideHailAgentInputData] = {
    val filePath = beamServices.beamConfig.beam.agentsim.agents.rideHail.initialization.filePath
    readFleetFromCSV(filePath, beamServices)
  }
   ...
   * @return list of [[beam.agentsim.agents.ridehail.RideHailAgent]] objects
   */
  private def readFleetFromCSV(filePath: String, beamServices: BeamServices): List[RideHailAgentInputData] = {
    try {
  ...
  def writeFleetData(beamServices: BeamServices, fleetData: Seq[RideHailAgentInputData]): Unit = {
    try {
      ...
      val filePath = beamServices.matsimServices.getControlerIO.getIterationFilename(
...
object RideHailFleetInitializer extends OutputDataDescriptor {
  ...
  override def getOutputDataDescriptions: java.util.List[OutputDataDescription] = {
    val filePath =
      GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getIterationFilename(0, outputFileBaseName + ".csv.gz")
    ...
    list.add(
      OutputDataDescription(getClass.getSimpleName.dropRight(1), relativePath, "id", "Id of the ride hail vehicle"))
    list.add(
...
case class Geofence(geofenceX: Double, geofenceY: Double, geofenceRadius: Double)
