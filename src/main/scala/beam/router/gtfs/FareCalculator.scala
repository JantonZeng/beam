...
class FareCalculator(directory: String) {
    ...
    def loadFares(feed: GTFSFeed): Unit = {
      ...
      feed.fares.forEach((id, fare) => {
        ...
        fare.fare_rules.forEach(r => {
          ...
          val rule: BeamFareRule =
            BeamFareRule(fares.get(r.fare_id).head, agencyId, r.route_id, r.origin_id, r.destination_id, r.contains_id)
  ...
  def getFareSegments(
    ...
    rules._2.groupBy(_.fare).view.filter(containsRule(_, routeId, _containsIds)).map(_._2.last)).map(f =>
      BeamFareSegment(f.fare, agencyId))
  }
...
object FareCalculator {
   ...
   */
  case class BeamFareSegment(fare: BeamFare, agencyId: String, patternIndex: Int, segmentDuration: Long)
  ...
  object BeamFareSegment {
    ...
    def apply(fareSegment: BeamFareSegment, patternIndex: Int, segmentDuration: Long): BeamFareSegment =
    ...
    def apply(fareSegment: BeamFareSegment, segmentDuration: Long): BeamFareSegment =
      new BeamFareSegment(fareSegment.fare, fareSegment.agencyId, fareSegment.patternIndex, segmentDuration)
  }
  ...
  def filterFaresOnTransfers(fareSegments: IndexedSeq[BeamFareSegment]): IndexedSeq[BeamFareSegment] = {
     ...
     */
    def groupFaresByAgencyAndProceed(fareSegments: IndexedSeq[BeamFareSegment]): IndexedSeq[BeamFareSegment] = {
     ...
     */
    def iterateTransfers(fareSegments: IndexedSeq[BeamFareSegment], trans: Int = 0): IndexedSeq[BeamFareSegment] = {
