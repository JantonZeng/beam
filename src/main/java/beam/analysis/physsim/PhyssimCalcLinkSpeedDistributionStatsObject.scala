...
object PhyssimCalcLinkSpeedDistributionStatsObject extends OutputDataDescriptor {
  ...
  def getOutputDataDescriptions: util.List[OutputDataDescription] = {
    val freeSpeedDistOutputFilePath =
      GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getIterationFilename(0, outputAsSpeedUnitFileName + ".csv")
    val freeSpeedDistAsPercetnageOutputFilePath =
      GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getIterationFilename(0, outputAsSpeedUnitFileName + ".csv")
    ...
    val freeSpeedDistAsPercetnageRelativePath = freeSpeedDistAsPercetnageOutputFilePath.replace(outputDirPath, "")
