...
object RideHailWaitingAnalysisObject extends OutputDataDescriptor {
  ...
  def getOutputDataDescriptions: util.List[OutputDataDescription] = {
    val rideHailWaitingStatsOutputFilePath: String =
      GraphsStatsAgentSimEventsListener.CONTROLLER_IO.getIterationFilename(0, fileName + ".csv")
    ...
    val rideHailIndividualWaitingTimesRelativePath: String =
      rideHailIndividualWaitingTimesOutputFilePath.replace(outputDirPath, "")
    ...
    list.add(OutputDataDescription(getClass.getSimpleName, rideHailWaitingStatsRelativePath, "Hour", "Hour of the day"))
    list.add(
      OutputDataDescription(
        getClass.getSimpleName,
        rideHailWaitingStatsRelativePath,
