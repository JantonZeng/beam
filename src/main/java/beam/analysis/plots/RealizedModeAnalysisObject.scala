...
object RealizedModeAnalysisObject extends OutputDataDescriptor {
  ...
  def getOutputDataDescriptions: util.List[OutputDataDescription] = {
    ...
    list.add(OutputDataDescription(getClass.getSimpleName, relativePath, "other", "Other modes of travel chosen"))
    list.add(
      OutputDataDescription(getClass.getSimpleName, relativePath, "ride_hail", "Ride Hail chosen as travel mode"))
